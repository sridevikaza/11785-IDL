...
class Autograd:
    ...
    def add_operation(self, 
        ...
        for input_array in inputs:
            self.gradient_buffer.add_spot(input_array)
    ...
    def backward(self, divergence):
        ...
        for operation_id, operation in enumerate(reversed(self.operation_list)):
        ...
        # self.gradient_buffer.get_param.
            if operation_id == 0:
                ...
                grad_of_output = self.gradient_buffer.get_param(operation.output)
        ...
        # NOTE: Make sure to unroll the inputs list if you aren't parsing a list in your backward.
            inputs = [input[0] for input in operation.inputs]
            # print(inputs)
            operation.backward_operation(grad_of_output, *inputs)
        ...
        # NOTE: Make sure the order of gradients align with the order of inputs
            ...
            for idx, input in enumerate(inputs):
                ...
                if operation.gradients_to_update[idx]:
                    operation.gradients_to_update[idx] = grad_of_output
                else:
                    self.gradient_buffer.update_param(input, grad_of_output)
